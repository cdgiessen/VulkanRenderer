...
//Testing out a water shader found on shadertoy (https://www.shadertoy.com/view/Ms2SD1) to see it in action
...
layout(location = 5) in vec3 fragCoord;
...
//#define SEA_TIME (1.0 + seaInput.sea.iTime * SEA_SPEED)
...
float map(vec3 p) {
    ...
    for(int i = 0; i < ITER_GEOMETRY; i++) {        
    	d = sea_octave((uv+(1.0 + inTime * SEA_SPEED))*freq,choppy);
    	d += sea_octave((uv-(1.0 + inTime * SEA_SPEED))*freq,choppy);
        h += d * amp;        
