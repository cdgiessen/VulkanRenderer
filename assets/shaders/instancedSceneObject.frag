...
float GeometrySmith(vec3 N, vec3 V, vec3 L, float roughness)
{
 ...
 vec3 PBR_Calc(vec3 N, vec3 V, vec3 F0, vec3 L, vec3 H, vec3 radiance){
	// cook-torrance brdf
     float NDF = DistributionGGX(N, H, 0.5f /*roughness*/);        
     float G   = GeometrySmith(N, V, L, 0.5f /*roughness*/);      
     vec3 F    = fresnelSchlick(max(dot(H, V), 0.0), F0);       
   
     vec3 kS = F;
     vec3 kD = vec3(1.0) - kS;
     kD *= 1.0 - 0.0f /*metallic*/;	  
     ...
     float NdotL = max(dot(N, L), 0.0);                
     return (kD * vec3(0.129, 0.404, 1.0f)/*albedo*/ / PI + specular) * radiance * NdotL; 
 }
