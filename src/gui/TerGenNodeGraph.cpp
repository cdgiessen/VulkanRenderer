...
//
//	template<typename T>
//	T Link<T>::GetValue(const int x, const int y, const int z, std::map<int, std::shared_ptr<INode>>& map) const {
//		if (input.ptr)
//			return map.at(input.id)->GetValue(x, y, z, data);
//		else
//			return data;
//	}
//
...
//
//	template<typename T>
//	bool Link<T>::SetInputNode(int id) {
//		input.id = id;
...
//	}
//
//	bool INode::GenerateNoiseSet(int seed, int numCells, glm::ivec2 pos, float scaleModifier) {
//		//does nothing. Never was supposed to. 
//	}
//
//	template<typename T>
//	Node<T>::Node(LinkType outputLinkType, std::map<int, std::shared_ptr<INode>>& map) : nodeMapRef(map), outputType(outputLinkType) {
//	}
//
//	template<typename T>
//	Node<T>::~Node() {}
...
//
//	template<typename T>
//	void Node<T>::SetNodeRefMap(std::map<int, std::shared_ptr<INode>>& map) {
//		nodeMapRef = map;
//	}
//
//	OutputNode::OutputNode(std::map<int, std::shared_ptr<INode>>& map) : Node<float>(LinkType::Float, map), input_output(LinkType::Float, 0) { }
...
//		return input_output.GetValue(x, y, z, nodeMapRef);
//	}
//
//	bool OutputNode::SetInputLink(int index, int id)  {
//		if (index == 0) 
//			if (input_output.GetLinkType() == nodeMapRef.at(id)->GetNodeType())
//				return input_output.SetInputNode(id);
//		return false;
//	}
//
...
//	}
//
//	ConstantFloatNode::ConstantFloatNode(float value, std::map<int, std::shared_ptr<INode>>& map) : Node<float>(LinkType::Float, map), value(LinkType::Float, value) {};
...
//	{
//		return value.GetValue(x,y,z, nodeMapRef);
//	}
...
//	}
//
//	bool ConstantFloatNode::SetInputLink(int index, int node) {
...
//	}
//
//	ConstantIntNode::ConstantIntNode(int value, std::map<int, std::shared_ptr<INode>>& map) : Node<int>(LinkType::Float, map), value(LinkType::Int, value) {};
...
//	}
//
//	bool ConstantIntNode::SetInputLink(int index, int node) {
//		return value.SetInputNode(node);
//	}
//
...
//	}
//
//	MathNode::MathNode(std::map<int, std::shared_ptr<INode>>& map) : Node<float>(LinkType::Float, map), input_a(LinkType::Float), input_b(LinkType::Float) {}
//	MathNode::~MathNode() { }
//	
...
//	}
//
//	bool MathNode::SetInputLink(int index, int node)
//	{
//		if (index == 0) 
...
//	}
//	
//	float AdditionNode::GetValue(const int x, const int y, const int z, float dummy)const	{ return input_a.GetValue(x, y, z, nodeMapRef) + input_b.GetValue(x, y, z, nodeMapRef); }
//	float SubtractNode::GetValue(const int x, const int y, const int z, float dummy)const	{ return input_a.GetValue(x, y, z, nodeMapRef) - input_b.GetValue(x, y, z, nodeMapRef); }
//	float MultiplyNode::GetValue(const int x, const int y, const int z, float dummy)const	{ return input_a.GetValue(x, y, z, nodeMapRef) * input_b.GetValue(x, y, z, nodeMapRef); }
//	float DivideNode::GetValue(const int x, const int y, const int z, float dummy)	const	{ return input_a.GetValue(x, y, z, nodeMapRef) / input_b.GetValue(x, y, z, nodeMapRef); }
//	float PowerNode::GetValue(const int x, const int y, const int z, float dummy)	const	{ return glm::pow(input_a.GetValue(x, y, z, nodeMapRef), input_b.GetValue(x, y, z, nodeMapRef)); }
//	float MaxNode::GetValue(const int x, const int y, const int z, float dummy)		const	{ return glm::max(input_a.GetValue(x, y, z, nodeMapRef), input_b.GetValue(x, y, z, nodeMapRef)); }
//	float MinNode::GetValue(const int x, const int y, const int z, float dummy)		const	{ return glm::min(input_a.GetValue(x, y, z, nodeMapRef), input_b.GetValue(x, y, z, nodeMapRef)); }
//
//
//	BlendNode::BlendNode(std::map<int, std::shared_ptr<INode>>& map)
//		: Node<float>(LinkType::Float, map), input_a(LinkType::Float), input_b(LinkType::Float), input_cutoff(LinkType::Float)  {} //input_blendAmount(LinkType::Float)
//
//
//	BlendNode::~BlendNode() {}
//	
...
//		float a = input_a.GetValue(x, y, z, nodeMapRef);
//		float b = input_b.GetValue(x, y, z, nodeMapRef);
//		float alpha = input_cutoff.GetValue(x, y, z, nodeMapRef);
//
//		return alpha * b + (1 - alpha) * a;
//	}
//
//	bool BlendNode::SetInputLink(int index, int node) {
//		if (index == 0)
//			return input_a.SetInputNode(node);
//		else if (index == 1)
//			return input_b.SetInputNode(node);
...
//	}
//
//	ClampNode::ClampNode(std::map<int, std::shared_ptr<INode>>& map) : Node<float>(LinkType::Float, map), input(LinkType::Float), lowerBound(LinkType::Float), upperBound(LinkType::Float) {}
//
//
//	ClampNode::~ClampNode() {}
//
...
//		
//		float value = input.GetValue(x, y, z, nodeMapRef);
//		float lower = lowerBound.GetValue(x, y, z, nodeMapRef);
//		float upper = upperBound.GetValue(x, y, z, nodeMapRef);
//
//		return glm::clamp(value, lower, upper);
//	}
//
//	bool ClampNode::SetInputLink(int index, int node) {
//		if (index == 0)
...
//
//
//	NoiseSourceNode::NoiseSourceNode(std::map<int, std::shared_ptr<INode>>& map)
//		: Node<float>(LinkType::Float, map), input_frequency(LinkType::Float, 0.1f)
...
//	}
//
//	bool NoiseSourceNode::SetInputLink(int index, int node) {
//
//		if (index == 0 && input_octaveCount.GetLinkType() == nodeMapRef.at(node)->GetNodeType())
//			return input_octaveCount.SetInputNode(node);
//		else if(index == 1 && input_persistance.GetLinkType() == nodeMapRef.at(node)->GetNodeType())
//			return input_persistance.SetInputNode(node);
//		else if(index == 2 && input_frequency.GetLinkType() == nodeMapRef.at(node)->GetNodeType())
...
//		: seed(seed), cellsWide(numCells), pos(pos), scale(scale), outputImage(numCells)
//	{           
//		
//		for (auto node : sourceGraph.GetNodeMap()) {
//			this->nodes[node.first] = std::make_shared<INode>(node.second);
//			this->nodes[node.first]->SetNodeRefMap(nodes);
//		}
//
//
//
//		for (auto noise : sourceGraph.GetNoiseSources()) {
//			nodes.at(noise)->GenerateNoiseSet(seed, cellsWide, glm::ivec2(pos.x * (cellsWide) / scale, pos.y * (cellsWide) / scale), scale / (cellsWide));
//		}
//
...
//	{
//		outputNode = std::make_shared<OutputNode>(nodes);
//		AddNode(outputNode);
//	}
//
//
...
//		nodes.insert(std::pair<int, std::shared_ptr<INode>>(node->GetID(), node));
//		return true;
//	}
//
...
//		node->SetID(GetNextID());
//		nodes.insert(std::pair<int, std::shared_ptr<INode>>(node->GetID(), node));
//		noiseSources.push_back(node->GetID());
//		return true;
//	}
//
//	bool TerGenNodeGraph::DeleteNode(int node) {
...
//	}
//
//	const std::map<int, std::shared_ptr<INode>>& TerGenNodeGraph::GetNodeMap() const {
//		return nodes;
//	}
//
//	const std::vector<int>& TerGenNodeGraph::GetNoiseSources() const {
//		return noiseSources;
//	};
//
