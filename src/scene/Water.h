...
class Water
{
	...
	public:
	Water (Resource::AssetManager& resourceMan, VulkanRenderer& renderer) : renderer (renderer)
	{
		mesh = create_water_plane_subdiv (6, 6);
	...
	void UpdateUniform (glm::vec3 camera_pos)
	{
		camera_pos.y = 0;
		ubo.model = glm::mat4 (1.0f);
		ubo.model = glm::translate (ubo.model, camera_pos);
		uniformBuffer->CopyToBuffer (&ubo, sizeof (ModelBufferObject));
	}
