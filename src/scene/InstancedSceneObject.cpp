...
void InstancedSceneObject::SetupPipeline ()
{
	...
	// Vertex::getAttributeDescriptions()); pipeMan.SetInputAssembly(mvp,
	...
	// (float)renderer.vulkanSwapChain.swapChainExtent.height, 0.0f, 1.0f, 0.0f, 0.0f);
	// pipeMan.SetScissor(mvp, renderer.vulkanSwapChain.swapChainExtent.width, renderer.vulkanSwapChain.swapChainExtent.height,
	// 0, 0); pipeMan.SetViewportState(mvp, 1, 1, 0); pipeMan.SetRasterizer(mvp,
	// VK_POLYGON_MODE_FILL, cullModeFlagBits, 	VK_FRONT_FACE_COUNTER_CLOCKWISE, VK_FALSE,
	// VK_FALSE, 1.0f, VK_TRUE); pipeMan.SetMultisampling(mvp, VK_SAMPLE_COUNT_1_BIT);
	// pipeMan.SetDepthStencil(mvp, VK_TRUE, VK_TRUE, VK_COMPARE_OP_GREATER, VK_FALSE, VK_FALSE);
	// pipeMan.SetColorBlendingAttachment(mvp, enableBlending, 	VK_COLOR_COMPONENT_R_BIT |
