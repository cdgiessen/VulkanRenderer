...
std::byte* TexData::GetDataPtr(){
	return data.data();
}

void TexResource::SetDataPtr(TexData* texData){
	dataPtr = texData;
}
...
void Manager::LoadTextureFromFile(TexID id){
	...
	for(int i = 0 ; i < texRes.dataDescription.layers; i++){
		...
		if (pixels == nullptr) {
			...
			<< desiredChannels << " but only " << texChannels << " channels\n";
		}
		else {

			std::memcpy(textureData.back()->GetDataPtr() + i * texWidth * texHeight * texChannels, 
				pixels, sizeof(std::byte) * texWidth * texHeight * texChannels);
