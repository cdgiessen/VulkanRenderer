...
void StartShaderCompilation(std::string str) {
	...
	for (const auto& entry : fs::directory_iterator (shaders_dir))
	{
		...
		if (entry.is_regular_file ())
		{
			    ...
			    p.extension () == ".tesc" || p.extension () == ".tese" || p.extension () == ".comp")
			{
				...
				out_path += ".spv";
#ifdef WIN32
			std::string cmd = "glslangvalidator.exe -V "s + in_path.generic_string() + " -o "s + out_path.generic_string();
#endif // WIN32
#ifdef UNIX
			std::string cmd = "glslangvalidator -V"s + in_path.generic_string() + " -o "s + out_path.generic_string();
#endif // UNIX
			    cmds.push_back(cmd);
			}
		}
