...
GPU_DoubleBuffer::GPU_DoubleBuffer (VulkanDevice& device, DescriptorManager& descriptor_man, RenderSettings& settings)
: device (device), descriptor_man (descriptor_man)
{
	for (auto& data : d_buffers)
	{
		...
		std::vector<DescriptorSetLayoutBinding> m_bindings = {
			{ DescriptorType::uniform_buffer, VK_SHADER_STAGE_ALL_GRAPHICS, 0, 1 },
			{ DescriptorType::uniform_buffer, VK_SHADER_STAGE_ALL_GRAPHICS, 1, 1 }
		};

		auto frameDatalayout = descriptor_man.CreateDescriptorSetLayout (DescriptorLayout (m_bindings));
		frameDataDescriptorSet = descriptor_man.CreateDescriptorSet (layout);
		...
		std::vector<DescriptorSetLayoutBinding> l_bindings = {
			{ DescriptorType::uniform_buffer, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 1 },
			{ DescriptorType::uniform_buffer, VK_SHADER_STAGE_FRAGMENT_BIT, 1, 1 },
			{ DescriptorType::uniform_buffer, VK_SHADER_STAGE_FRAGMENT_BIT, 2, 1 }
		};
		auto lightingLayout = descriptor_man.CreateDescriptorSetLayout (DescriptorLayout (l_bindings));
		lightingDescriptorSet = descriptor_man.CreateDescriptorSet (lightingLayout);
