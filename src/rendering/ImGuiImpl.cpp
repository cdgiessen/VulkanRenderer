...
// of using this. If you use this binding you'll need to call 5 functions: ImGui_ImplXXXX_Init(),
// ImGui_ImplXXX_CreateFontsTexture(), ImGui_ImplXXXX_NewFrame(), ImGui_ImplXXXX_Render() and
...
{
	if (g_CheckVkResult) g_CheckVkResult (err);
}
...
void ImGui_ImplGlfwVulkan_RenderDrawLists (ImDrawData* draw_data)
{
	...
	if (!g_VertexBuffer[g_FrameIndex] || g_VertexBufferSize[g_FrameIndex] < vertex_size)
	{
		...
		alloc_info.memoryTypeIndex =
		    ImGui_ImplGlfwVulkan_MemoryType (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, req.memoryTypeBits);
		err = vkAllocateMemory (g_Device, &alloc_info, g_Allocator, &g_VertexBufferMemory[g_FrameIndex]);
		ImGui_ImplGlfwVulkan_VkResult (err);
		err = vkBindBufferMemory (
		    g_Device, g_VertexBuffer[g_FrameIndex], g_VertexBufferMemory[g_FrameIndex], 0);
		ImGui_ImplGlfwVulkan_VkResult (err);
	...
	// Bind pipeline and descriptor sets:
	{
		...
		vkCmdBindDescriptorSets (
		    g_CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, g_PipelineLayout, 0, 1, desc_set, 0, NULL);
	}
	...
	// Setup scale and translation:
	{
		...
		translate[1] = -1.0f;
		vkCmdPushConstants (
		    g_CommandBuffer, g_PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof (float) * 0, sizeof (float) * 2, scale);
		vkCmdPushConstants (
		    g_CommandBuffer, g_PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, sizeof (float) * 2, sizeof (float) * 2, translate);
	}
...
void ImGui_ImplGlfwVulkan_KeyCallback (GLFWwindow*, int key, int scancode, int action, int mods)
{
	ImGuiIO& io = ImGui::GetIO ();
...
bool ImGui_ImplGlfwVulkan_CreateFontsTexture (VkCommandBuffer command_buffer)
{
	...
	// Create the Image:
	{
		...
		alloc_info.allocationSize = req.size;
		alloc_info.memoryTypeIndex =
		    ImGui_ImplGlfwVulkan_MemoryType (VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, req.memoryTypeBits);
...
void PrepareImGui (Window* window, VulkanDevice* device, VkRenderPass renderpass)
{
	...
	std::vector<VkDescriptorPoolSize> pool_size = { { VK_DESCRIPTOR_TYPE_SAMPLER, maxSets },
	...
	VK_CHECK_RESULT (vkCreateDescriptorPool (device->device, &pool_info, VK_NULL_HANDLE, &init_data.descriptor_pool));

	init_data.allocator = VK_NULL_HANDLE;
	init_data.gpu = device->physical_device.physical_device;
	init_data.device = device->device;
	...
	ImGui_ImplGlfwVulkan_CreateFontsTexture (cmdBuf.Get ());
