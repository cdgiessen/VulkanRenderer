...
: renderer (renderer)
{
	...
	uint32_t vBufferSize = static_cast<uint32_t> (mesh->vertexData.size ()) * sizeof (float);
	uint16_t iBufferSize = static_cast<uint16_t> (mesh->indexData.size ()) * sizeof (uint16_t);


	vmaVertices = std::make_unique<VulkanBufferVertex> (renderer.device, (uint32_t)mesh->vertexData.size ());
	vmaIndicies = std::make_unique<VulkanBufferIndex> (renderer.device, (uint16_t)mesh->indexData.size ());

	auto vertexStagingBuffer =
	    std::make_shared<VulkanBufferStagingVertex> (renderer.device, (uint32_t)vertexCount,
	                                                                        mesh->vertexData.data ());
	auto indexStagingBuffer = std::make_shared<VulkanBufferStagingIndex> (renderer.device, (uint32_t)indexCount,
	                                                                      mesh->indexData.data ());
	...
	renderer.SubmitWork (WorkType::transfer, work, {}, {},
	                     { vertexStagingBuffer, indexStagingBuffer }, { readyToUse });
}
