...
VulkanBufferStagingVertex::VulkanBufferStagingVertex (VulkanDevice& device,
...
VulkanBufferIndex::VulkanBufferIndex(VulkanDevice& device, uint32_t count) :
	VulkanBuffer(device, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
		sizeof(uint32_t) * count, (VkBufferUsageFlags)(VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT),
		VMA_MEMORY_USAGE_GPU_ONLY), is32bit(true) {}

VulkanBufferIndex::VulkanBufferIndex(VulkanDevice& device, uint16_t count) :
	VulkanBuffer(device, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
		sizeof(uint16_t) * count, (VkBufferUsageFlags)(VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT),
		VMA_MEMORY_USAGE_GPU_ONLY), is32bit(false) {}
...
VulkanBufferStagingIndex::VulkanBufferStagingIndex(VulkanDevice& device, uint32_t count, void* pData) :
	VulkanBuffer(device, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
		sizeof(uint32_t) * count, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
		VMA_MEMORY_USAGE_CPU_ONLY, VMA_ALLOCATION_CREATE_MAPPED_BIT, pData), is32bit(true)
{}
