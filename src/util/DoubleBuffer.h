...
template <typename T, int size> class DoubleBufferArray
{
	public:
	DoubleBufferArray ();
...
template <typename T, int size> DoubleBufferArray<T, size>::DoubleBufferArray () : which (false)
{
	std::fill (std::begin (freeElems), std::end (freeElems), false);
}

template <typename T, int size> int DoubleBufferArray<T, size>::Allocate ()
{
...
template <typename T, int size> void DoubleBufferArray<T, size>::Free (int index)
{
	std::unique_lock<std::shared_mutex> lock (writeLock);
...
template <typename T, int size> T DoubleBufferArray<T, size>::Read (int index)
{
	std::shared_lock<std::shared_mutex> lock (writeLock);
	return (*currentRead)[index];
}

template <typename T, int size> std::array<T, size>* DoubleBufferArray<T, size>::ReadAll ()
{
	std::shared_lock<std::shared_mutex> lock (writeLock);
	return currentRead;
}

template <typename T, int size> void DoubleBufferArray<T, size>::Write (int index, T data)
{
...
template <typename T, int size> void DoubleBufferArray<T, size>::Swap ()
{
