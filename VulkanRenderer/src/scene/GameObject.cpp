...
void GameObject::SetupPipeline() {
	...
	pipeMan.SetVertexShader(mvp, loadShaderModule(renderer->device.device, "shaders/gameObject_shader.vert.spv"));
	pipeMan.SetFragmentShader(mvp, loadShaderModule(renderer->device.device, "shaders/gameObject_shader.frag.spv"));
