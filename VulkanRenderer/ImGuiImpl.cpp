...
//		uploadSize));
//
//	stagingBuffer.map(device->device);
...
//	VkPushConstantRange pushConstantRange = initializers::pushConstantRange(VK_SHADER_STAGE_VERTEX_BIT, sizeof(ImGuiPushConstBlock), 0);
...
//
//	vertexShaderModule = loadShaderModule(device->device, "shaders/ui.vert.spv");
//	fragmentShaderModule = loadShaderModule(device->device, "shaders/ui.frag.spv");
//
...
//	vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(ImGuiPushConstBlock), &pushConstBlock);
//
//	// Render commands
//	ImDrawData* imDrawData = ImGui::GetDrawData();
